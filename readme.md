...
This is a tray icon tool for MeshCentral. It displays the state of the agent, you can start and stop the agent and request help.
...
-visible                     When starting on the system tray MeshCentral Assistant will start with the dialog box visible on the primary screen.
...
The .msh file contains the policy MeshCentral Assistant built-in agent will follow. It includes which server to connect to, how to authenticate the server, which device group to initially join, etc. A sample .msh file will look like this and may be included within the MeshCentral Assistant executable when downloaded from the server.
...
Here is a list of the keys currently supported by the agent. Note that the key name must have the exact capitalization:
...
Title                        When specified, this is a branded title used for the MeshCentral Assistant dialog box.
...
## Video Tutorials
You can watch many tutorial videos on the [MeshCentral YouTube Channel](https://www.youtube.com/channel/UCJWz607A8EVlkilzcrb-GKg/videos). MeshCentral Assistant has its own introduction video.
